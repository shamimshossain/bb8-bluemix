...
function start(deviceId, apiKey, apiToken, mqttHost, mqttPort, callback) {
  ...
  var clientId = ['d', org, 'sphero', deviceId].join(':');
...
module.exports = function() {
  return {
    "connect" : function(callback) {
      properties.parse('./sphero-config.properties', {path: true}, function(err, cfg) {
        if (err) {
          console.error('A file named sphero-config.properties containing the device registration from the IBM IoT Cloud is missing.');
